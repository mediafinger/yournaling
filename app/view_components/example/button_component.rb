...
module Example
  ...
  # when `href:` is given.
  #
  #   Example::ButtonComponent.new("Save memory", variant: :primary)
  #   Example::ButtonComponent.new("Cancel", variant: :ghost, href: "/back")
  #   Example::ButtonComponent.new("Delete", variant: :danger, icon: "trash")
  #   = render(Example::ButtonComponent.new(variant: :secondary, size: :lg)) { "Browse" }
  #
  ...
  class ButtonComponent < BaseComponent
    ...
    slim_template <<~SLIM
      - if href.present?
        = link_to(href, class: css_class, role: "button", aria: { disabled: disabled.presence }) do
          - if icon
            = render(Example::IconComponent.new(icon, size: icon_size))
          span = label
          - if trailing_icon
            = render(Example::IconComponent.new(trailing_icon, size: icon_size))
      - else
        button type=type class=css_class disabled=disabled
          - if icon
            = render(Example::IconComponent.new(icon, size: icon_size))
          span = label
          - if trailing_icon
