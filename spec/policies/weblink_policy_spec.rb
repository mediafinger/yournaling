...
RSpec.describe WeblinkPolicy do
  let(:user) { FactoryBot.create(:user) }
  let(:team) { FactoryBot.create(:team) }
  let(:other_team) { FactoryBot.create(:team) }
  let(:owned_record) { FactoryBot.create(:weblink, team: team) }
  let(:unassociated_record) { FactoryBot.create(:weblink, team: other_team) }
  let(:policy_class) { described_class }

  describe "#scope" do
    let(:relation) { Weblink.unscoped }
