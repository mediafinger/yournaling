...
RSpec.describe ThoughtPolicy do
  let(:user) { FactoryBot.create(:user) }
  let(:team) { FactoryBot.create(:team) }
  let(:other_team) { FactoryBot.create(:team) }
  let(:owned_record) { FactoryBot.create(:thought, team: team) }
  let(:unassociated_record) { FactoryBot.create(:thought, team: other_team) }
  let(:policy_class) { described_class }

  describe "#scope" do
    let(:relation) { Thought.unscoped }
