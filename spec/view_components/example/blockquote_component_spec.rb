...
RSpec.describe Example::BlockquoteComponent, type: :component do
  it "renders a figure with a blockquote" do
    rendered = render_inline(described_class.new("We travel to lose ourselves."))

    expect(rendered).to have_css("figure.ex-blockquote blockquote", text: "We travel to lose ourselves.")
  end
  ...
  it "renders attribution in a figcaption when cite is given" do
    ...
    expect(rendered).to have_css("figure.ex-blockquote figcaption", text: "— Field notes")
  end
  ...
  it "adds the card modifier class" do
    ...
    expect(rendered).to have_css("figure.ex-blockquote.ex-blockquote--card")
  end
