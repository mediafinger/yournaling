...
RSpec.describe Yui::ToastComponent, type: :component do
  it "renders the message with a live-region role and a dismiss control" do
    ...
    expect(rendered).to have_css("div.yui-toast.yui-toast--success[role='status'] p.yui-toast__body", text: "Memory saved")
    expect(rendered).to have_css("button.yui-toast__dismiss[aria-label='Dismiss'][data-action='yui-toast#dismiss']")
  ...
  it "uses an assertive alert for the danger variant" do
    ...
    expect(rendered).to have_css(".yui-toast--danger[role='alert'][aria-live='assertive']")
  end
