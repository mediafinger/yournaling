...
RSpec.describe Yui::LabelComponent, type: :component do
  it "renders a <label> bound to the given control id" do
    rendered = render_inline(described_class.new("Title", for: "memory_title"))

    expect(rendered).to have_css("label.yui-label[for='memory_title']", text: "Title")
  end
  ...
  it "shows a required marker hidden from assistive tech" do
    ...
    expect(rendered).to have_css(".yui-label__required[aria-hidden='true']", text: "*")
  end
  ...
  it "shows an optional marker instead when optional: true" do
    ...
    expect(rendered).to have_css(".yui-label__optional", text: "(optional)")
    expect(rendered).to have_no_css(".yui-label__required")
  end
end
